#!/usr/bin/env bash
# Trackerless-local test (#57): a repo with no origin remote whose artifact is a
# facts/ + sources/ + refs/ corpus is NOT a GitHub tracker. The plan must classify
# it trackerless (golden), recognize the corpus (source_of_truth=facts-corpus), and
# force NO tracker machinery; apply must wire the corpus config (domain + stamp +
# CLAUDE block) and skip labels + the two tracker docs.
...
assert_contains "$GOT" '"lifecycle_overlay": "none"'       "no lifecycle overlay forced on a corpus"
