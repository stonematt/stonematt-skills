...
3. For each issue: `gh issue edit <N> --repo <owner/repo> --add-label "status: staged" --remove-label "status: wip" --remove-label "status: ready"`. Strip both upstream lanes — tickets that were implemented autonomously often skip `wip` and merge straight from `status: ready`; removing a label the issue doesn't carry is a harmless no-op (both labels exist in the repo).
